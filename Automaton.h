...
int Automaton::randomChoice(const int begin, const int end)							//choice with equal probability
{
	std::random_device random_dev;										//random number generator that produces non-deterministic random numbers
	std::mt19937 generator(random_dev());
	std::uniform_int_distribution<int>  distribution(begin, end);
	return distribution(generator);
}

int Automaton::randomWithWeights()										//choice with probability based on weights
{
	srand(time(0));
	int prob = rand() % 10;
